...
**Version:** 0.2 (PDK reality alignment + priority labels pending)
...
**REQ-SI-001:** SPI master shall support clock up to 5 MHz, **mode 0 only** (amended 2026-05-29; FRAM target FM25V20A supports mode 0; multi-mode reserved for v1.1)
...
**REQ-SI-010:** Power switching GPIOs (7×) shall drive up to **24 mA** each on-die (amended 2026-05-29; the workshop slot's `bi_24t` pad is 24 mA — the prior "50 mA" claim was inconsistent with the PDK). External MOSFET driven by the 24 mA pad can extend to 50+ mA where required by a sensor load.
...
|------|------|-------|-----|-------|-------------|
| 0x00 | CTRL | 16 | RW | 0x0000 | Bit 0: chip_enable. Bits 1-3: reserved (deprecated mode-request from early design). Bits 4-7: wake_period_sel (consumed by wake_timer). Bit 8: IS_in_normal_enable — when 1, NORMAL cycles run an IS sweep after the sensor read; when 0, NORMAL cycles read sensors only. Bit 9: alerts_enabled — when 1, alert_engine evaluates and latches threshold violations. Must remain 0 until ESP32 firmware has confirmed that sensor registers hold valid measurements (default reset values would otherwise satisfy below-threshold comparators spuriously). Bits 10-15: reserved. |
...
- Clock: up to 250 kHz (was "10 MHz" in v0.x; amended 2026-05-29 after audit found that the synchronous SPI slave clocked from the 1 MHz main domain cannot meet Nyquist for SCK above ~500 kHz. ESP32 firmware paces register accesses at minutes-scale; 250 kHz × 24-bit transactions = 96 µs each, negligible vs the 1-hour wake cycle.)
- Bit order: MSB first
