...
Usage:
  ...
  python golden_model.py --is-demo       Demo IS measurement on baseline vs activity proxy
"""
...
class SoilState:
    ...
    activity_factor: float = 0.0     # 0.0 (baseline) to 1.0 (high proxy activity)
...
class RandlesSoilModel:
    ...
    Parameters derived from:
      ...
      - Generic microbial impedance studies

    Activity proxy effect: reduces R_ct (charge transfer), increases C_DL.
    ...
    def _compute_parameters(self):
        ...
        # Activity proxy increases capacitance.
        self.C_DL = base_c_dl * (1 + 5 * s.activity_factor)

        # R_ct: charge transfer resistance
        # Baseline soil: 1000-10000 ohm
        # Activity proxy reduces this value.
        base_r_ct = 10000.0 / (1 + s.organic_matter / 3.0)
        self.R_ct = base_r_ct / (1 + 20 * s.activity_factor)

        # Warburg coefficient (diffusion-limited impedance)
        self.sigma_W = 1000.0 * (1 - 0.5 * s.activity_factor)
...
def scenario_bioactive_soil(severity: float = 0.7):
    """Soil with elevated bioactivity proxy."""
    return SoilState(
        moisture_vwc=40,
        temperature_c=24,
        salinity_ec=1.5,
        activity_factor=severity,
        organic_matter=3.0,
        clay_fraction=0.3,
    )
...
def test_randles_physics():
    ...
    # Bioactive soil proxy should have different R_ct
    soil_bioactive = RandlesSoilModel(scenario_bioactive_soil(0.7))
    z_bioactive_30k = soil_bioactive.impedance(3e4)
    z_healthy_30k = soil_healthy.impedance(3e4)

    # Bioactive proxy has reduced R_ct -> different impedance at mid freq
    diff_pct = abs(z_bioactive_30k - z_healthy_30k) / abs(z_healthy_30k) * 100
    assert diff_pct > 5, f"Bioactive vs baseline should differ >5%, got {diff_pct:.1f}%"
...
# ============================================================
# 15. IS demo: baseline vs activity proxy
# ============================================================

def is_demo():
    """Demonstrate IS discriminating baseline vs activity proxy."""
    print("=" * 70)
    print("IS MEASUREMENT DEMO — Baseline vs Bioactive Soil Proxy")
    print("=" * 70)
    print()
    ...
    scenarios = [
        ("Baseline, 32% VWC", scenario_healthy_soil()),
        ("Bioactive (severity 0.3)", scenario_bioactive_soil(0.3)),
        ("Bioactive (severity 0.7)", scenario_bioactive_soil(0.7)),
        ("Bioactive (severity 1.0)", scenario_bioactive_soil(1.0)),
    ...
    print()
    print("Observations:")
    print("  - Baseline vs activity proxy differs most at 30 kHz in this model")
    print("  - Activity proxy reduces R_ct -> lower |Z| at mid frequencies")
    print("  - This validates the digital IS data path, not a biological claim")
    print()
...
def main():
    ...
    parser.add_argument("--is-demo", action="store_true",
                       help="Demo IS baseline vs activity proxy")
